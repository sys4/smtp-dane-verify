...
Problems with DANE usally stem from TLSA Resource Records that don't match the
SMTP services x509 certificate fingerprint. This happens if not only the
...
Whatever reason if no TLSA Resource Record matches DANE verification will fail
...
service that verifies one of more TLSA Resource Records, published for a SMTP
...
host machine. To start smtp-tlsa-verify either change into
...
This will enable *and* start the service. If you start `smtp-tlsa-verify` for
the first time, docker will initially download the container
`sys4ag/smtp-tlsa-verify` and then it will start it. If this step was
successfull you will be able to verify `smtp-tlsa-verify` has bound itself to
...
Your `smtp-tlsa-verify` docker container should now be ready to probe hosts.
...
Upon return `smtp-tlsa-verify` will output
...
. Give the Monitor a menu:[Friendly Name] e.g. `SMTP TLSA Verify`.
