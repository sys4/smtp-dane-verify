...
SMTP-DANE-Verify aims to provide an overview what *exactly* is wrong.
Therefore it will not stop it's testing if e.g. DNSSEC-verification fails, but
all the TLSA record MX service tests succeed. We consider this useful e.g. if a
platform is on the way to become fully DANE compliant, but for whatever reason
not there yet.


== How do I use SMTP-DANE-Verify?

Either you integrate SMTP-DANE-Verify, it was written in Python, in your own
service environment or you pull and run SMTP-DANE-Verify as a docker container,
request test runs on your monitoring platform from the service running inside
the container and handle the test results in your monitoring.

In both cases SMTP-DANE-Verify uses it's own
https://pypi.org/project/smtp-dane-verify/[smtp-dane-verify^] Python library to
provide and deliver the verification service. If you are a skilled operator or
developer you may want to integrate the Python library into your own project /
environment. If you don't want to do so you may just run SMTP-DANE-Verify as a
docker container. In this case the following section should get you up and
running within short time.
...
The use e.g. the `lsof` command to verify the service listens for client
connections:
...
provides a template you may install and use in combination with hosts you want
to verify. The template assumes you run : configures Zabbix to request SMTP DANE Verification from a smtp-dane-verify
service running on a remote host (here: 192.0.2.210)
...
----
  ...
  templates:
    -
      template: Template_SMTP_TLSA_Verify
      name: Template_SMTP_TLSA_Verify
      groups:
        - name: Templates
      items:
        -
          name: SMTP_DANE_Verify
          type: HTTP_AGENT
          key: 'check.dane["example.com"]'
          delay: 4h
          trends: '0'
          value_type: TEXT
          url: 'http://192.0.2.210:8801/verify/?api_key=SAsMTaxKPbTZwD0c25cCZEJXJAtqif'
          posts: '{"domain":"example.com"}'
          post_type: JSON
          request_method: POST
          tags:
            -
